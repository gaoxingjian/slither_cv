...
def callerVisibilityHavePublic(function, callGraph):
    ...
    #     print("马明亮")``
    #     print(functionNode)
    #     print(functionNode.fathers)
    for father in functionNode.fathers:
        if father.function.visibility == 'public':
            return True
        if callerVisibilityHavePublic(father.function, callGraph):
            return True
    return False
        ...
        for function in contract.functions:
            if function.is_implemented:
                    ...
                    if cfgCandidateAllPath_Node:   # 证明cfg本身就找到Reentrancy了，准备humanlook, 注意reversed, DM
                        ...
                        if privateVisibility is True:
                            if havePublicCaller is True:
                                reentrancyFlag = True
                                print('\t\tcontract: {} | function: {} | private: {} | publicCaller: {} | Execution Locks and Eth money balance modification: {}'.format(
                            ...
                            reentrancyFlag = True
                            print(
                                '\t\tcontract: {} | function: {} | private: {} | Execution Locks and Eth money balance modification: {}'.format(
                ...
                if reentrancyFlag is False:  # 证明cfg本身没找到Reentrancy
                    ...
                    '''
                    print('\t\tCFG analysis is safe, so start analyzing XCFG'.format(function.full_name))
                        ...
                        if icfgCandidateAllPath_Node:    # 证明icfg本身找到Reentrancy了，准备humanlook, DM
                            ...
                            if privateVisibility is True:
                                if havePublicCaller is True:
                                    reentrancyFlag = True
                                    print(
                                        '\t\tcontract: {} | function: {} | private: {} | publicCaller: {} | Execution Locks and Eth money balance modification: {}'.format(
                                            function.contract.name, function.full_name, privateVisibility, havePublicCaller, advanceUpdateFlag))
